...
namespace DevelopmentWithADot.SPExpressionBuilders
{
	public abstract class ConvertedExpressionBuilder : ExpressionBuilder
	{
		...
		protected static Object Convert(Object value, Type destinationType)
		{
			...
			if ((destinationType.IsInstanceOfType(value) == true) || (value.GetType().GetInterfaces().Contains(destinationType) == true))
			{
				return (value);
			}
			...
			if ((typeof(IConvertible).IsAssignableFrom(destinationType) == true) && (value is IConvertible))
			{
			...
			if ((conversionOperator.ReturnType == destinationType) && (conversionOperator.GetParameters().Length == 1) && (conversionOperator.GetParameters()[0].ParameterType.IsInstanceOfType(value) == true))
			{
